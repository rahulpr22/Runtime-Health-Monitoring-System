...
public class Grammar {
	 ...
	 */
	public Grammar(Scanner grammarDescription) throws GrammarException {
		grammarScanner = grammarDescription;
		...
		while (grammarScanner.hasNext()) {
			...
			if (line[0].equalsIgnoreCase("%Tokens")) {
				...
				if (startVariable == null) {
					throw new GrammarException("Start variable was not declared");
				}
				...
				while (grammarScanner.hasNext()) {
					...
					if (ruleLine[0].charAt(0) != '<') {
						throw new GrammarException("Unexpected " + ruleLine[0] + " on line " + lineCount);
					}

					Variable leftSide = variableForIdentifier(ruleLine[0]);

					if (leftSide == null) {
						throw new GrammarException("Variable " + ruleLine[0] + " referenced on line " + lineCount + " was not declared");
					}
				...
				throw new GrammarException("Unexpected " + line[0] + " on line " + lineCount);
			}

			lineCount++;
		}
		/*for(Rule v: rules){
			System.out.println("has left recursion --> "+v.hasLeftRecursion());
			System.out.println(v.getLeftSide()+" : ");
			for(RuleElement r : v.getRightSide())
